...
public partial class McWorld : UdonSharpBehaviour
{
    ...
    // CODE-OWNED TUNING: all perf budget/timing knobs below carry [System.NonSerialized] so the
    // CODE initializer is the single source of truth — scene/inspector values are IGNORED for
    // these. (They were previously scene-serialized, which silently overrode every code-side
    // tuning change; see the adaptive-budget THROUGHPUT FIX that the scene had reverted.)
    // CHUNK GO POOL: render GameObjects Instantiated per frame during the startup pool pre-warm.
    [System.NonSerialized] public int chunkGoPrewarmPerFrame = 24;
    ...
    // CHUNK GO POOL: chunk slots scanned per frame for render GameObjects to recycle.
    [System.NonSerialized] public int chunkGoRecycleScanPerFrame = 32;
    ...
    // Budget per incremental GPU mesh decode step (ms).
    [System.NonSerialized] public float gpuMeshDecodeStepBudgetMs = 5.0f;
    // Max incremental GPU mesh decode steps per chunk per frame.
    [System.NonSerialized] public int gpuMeshDecodeStepsPerFrame = 2;
    // Total GPU mesh decode budget per frame (ms, aggregate across chunks).
    [System.NonSerialized] public float gpuMeshDecodeFrameBudgetMs = 5.0f;
    ...
    // Deferred interior chunks re-queued per frame (never-skip guarantee).
    [System.NonSerialized] public int deferredInteriorMeshRequestsPerFrame = 8;
    // Chunk slots scanned per frame for deferred interior meshes to wake.
    [System.NonSerialized] public int deferredInteriorMeshScanCountPerFrame = 256;
    ...
    // Max nearby chunk colliders enabled per frame before the general deferred-work scan runs.
    [System.NonSerialized] public int nearColliderUpdatesPerFrame = 8;
    // Deferred secondary tasks finished per frame when there is headroom.
    ...
    [System.NonSerialized] public int gpuResidentSyncsPerFrame = 32;
    ...
    private const byte BLOCK_FIRE = 51;
    private const byte BLOCK_FURNACE_LIT = 62;
    private const byte BLOCK_PORTAL = 90;
    // Max ambient-particle emitters tracked per chunk (torch/fire/lava-surface/furnace/portal).
    // 512 ints = 2KB per scanned chunk; overflow just thins ambient particles on extreme chunks.
    public const int AMBIENT_EMITTER_CAP = 512;
    ...
    // PAC PHASE BREAKDOWN: per-frame time inside each ProcessActiveChunks phase, so 100% of
    // "Active processing" is attributed (the load-phase logs had ~30ms/frame unaccounted).
    private float stats_pacMaintainMs = 0f;   // prewarm + lookup flushes + _GpuMaintainResidentChunks
    private float stats_pacLightingMs = 0f;   // _ProcessGpuLighting (both calls)
    private float stats_pacReadbackMs = 0f;   // _ProcessGpuFaceReadback
    private float stats_pacDataGenMs = 0f;    // StepChunkDataGeneration loop
    private float stats_pacNearMeshMs = 0f;   // near-mesh guarantee lane
    ...
    private int stats_rqHealInterior = 0;   // border-heal inside _ScheduleDeferredInteriorMeshUpdates
    private int stats_rqFollowup = 0;       // pendingChunkMeshRebuild followups (meshing-loop cleanup)
    private int stats_rqNeighborTrigger = 0; // TriggerNeighborMeshRebuilds invocations (up to 6 requests each)
    ...
    // EVENT-DRIVEN RESIDENT MAINTAIN: the full radial ring scan below visits ~R^2*pi*worldDimY
    // chunk positions (hundreds) every frame just to find the ~1 chunk that needs an atlas sync —
    // measured 8-11ms/frame of pure Udon scanning in the PAC phase logs. Chunks that finish
    // data-gen now enqueue themselves; the queue is drained cheaply every frame, and the full
    // safety scan only runs when the player's chunk changes or a periodic cooldown expires
    // (covers slot eviction recovery and anything the queue missed).
    private int[] _gpuSyncRequestQueue;
    private int _gpuSyncRequestCount = 0;
    private int _gpuResidentScanCooldown = 0;
    private int _gpuResidentLastCenterX = int.MinValue;
    private int _gpuResidentLastCenterZ = int.MinValue;
    private const int GPU_SYNC_QUEUE_CAP = 256;
    private const int GPU_RESIDENT_SCAN_COOLDOWN_FRAMES = 30;

    private void _GpuQueueResidentSync(ChunkData chunk)
    {
        if (chunk == null) return;
        int ci = ChunkCenteredCoordsTo1D(chunk.chunkX_world, chunk.chunkY_world, chunk.chunkZ_world);
        if (ci < 0) return;
        if (_gpuSyncRequestQueue == null) _gpuSyncRequestQueue = new int[GPU_SYNC_QUEUE_CAP];
        if (_gpuSyncRequestCount >= GPU_SYNC_QUEUE_CAP) return; // overflow: the periodic safety scan catches it
        _gpuSyncRequestQueue[_gpuSyncRequestCount] = ci;
        _gpuSyncRequestCount = _gpuSyncRequestCount + 1;
    }
    ...
    private void _GpuMaintainResidentChunks(float frameStart, float frameBudget)
    {
        ...
        // Drain the pending-sync queue first (newly generated chunks). Cheap: O(pending).
        // Entries are consumed front-to-back; on budget exhaustion the tail shifts down and
        // waits for next frame. Skipped/invalid entries are simply dropped — the periodic
        // safety scan below is the correctness backstop for anything missed.
        if (_gpuSyncRequestCount > 0 && chunks_1D != null)
        {
            int consumed = 0;
            while (consumed < _gpuSyncRequestCount)
            {
                if (touched >= syncBudget) break;
                if (Time.realtimeSinceStartup - frameStart > frameBudget) break;
                int ci = _gpuSyncRequestQueue[consumed];
                consumed++;
                if (ci < 0 || ci >= chunks_1D.Length) continue;
                ChunkData qc = chunks_1D[ci];
                if (qc == null || !qc.isDataReady) continue;
                int qSlot = gpuChunkIndexToSlot[ci];
                if (qSlot >= 0 && qSlot < gpuChunkSlotCapacity) continue; // already lit
                int qdx = qc.chunkX_world - gpuResidentCenterChunkX;
                int qdz = qc.chunkZ_world - gpuResidentCenterChunkZ;
                if (qdx * qdx + qdz * qdz > rr) continue; // outside lit radius — scan handles later
                byte[] qData = _GetDecompressedData(qc);
                if (qData == null) continue;
                _GpuSyncChunkBlocks(qc, qData);
                touched++;
            }
            int remaining = _gpuSyncRequestCount - consumed;
            for (int mi = 0; mi < remaining; mi++) _gpuSyncRequestQueue[mi] = _gpuSyncRequestQueue[consumed + mi];
            _gpuSyncRequestCount = remaining;
        }

        // Full radial safety scan: only when the player's chunk changed (immediate re-light of
        // newly-in-range columns) or the periodic cooldown expires (eviction recovery, missed
        // entries). Skipping it on ordinary frames is the entire win.
        bool centerMoved = gpuResidentCenterChunkX != _gpuResidentLastCenterX
            || gpuResidentCenterChunkZ != _gpuResidentLastCenterZ;
        _gpuResidentScanCooldown = _gpuResidentScanCooldown - 1;
        if (!centerMoved && _gpuResidentScanCooldown > 0) return;
        _gpuResidentLastCenterX = gpuResidentCenterChunkX;
        _gpuResidentLastCenterZ = gpuResidentCenterChunkZ;
        _gpuResidentScanCooldown = GPU_RESIDENT_SCAN_COOLDOWN_FRAMES;
    ...
    // Derived-scan Y-rows per prep step. Chunks are 16 tall (NOT 64 — old comment was stale), so
    // the previous value of 16 degenerated the "time-sliced" scan into ONE atomic 4096-voxel Udon
    // pass (~8-15ms) per prep step — the un-attributed frame spikes in the load-phase perf logs.
    ...
    // FLUID-V2 diagnostics: prove which scan path each fluid cell took + where stage-3 time
    // actually lives (setup = self/neighbor decompress + churn-mask scan vs the sliced scan).
    private float stats_fluidSetupMs = 0f;
    private int stats_fluidBuilds = 0;
    private int stats_fluidCellsEnclosed = 0;
    private int stats_fluidCellsSurf = 0;
    private int stats_fluidCellsFull = 0;
#endif
    ...
    private void _StepGpuChunkPrepImpl(ChunkData chunk)
    {
        ...
        {
            // Derived caches already fresh for this exact data version — typical for the
            // neighbor-rebuild re-preps that flood the meshing loop during load. Skip the
            // whole 4096-voxel rescan; the flags/emitters/columns can't have changed.
            if (chunk._derivedScanCursor == 0 && !chunk._derivedDirty
                && chunk._derivedForDataVersion == chunk._cachedDataVersion)
            {
                chunk._gpuPrepStage = 2;
                return;
            }
        ...
        if (chunk._gpuPrepStage == 3)
        {
            ...
            if (chunk._gpuFluidZCursor < 0)
            {
...
#endif
                // Setup: detach the shared voxel mesh, then decide build-vs-defer.
                if (fluidMf != null && fluidMf.sharedMesh == gpuVoxelChunkMesh) fluidMf.sharedMesh = null;
                // FLUID BUILD NOW + ONE HEAL: water builds IMMEDIATELY (missing chunks read as
                // still water — faces culled, not walled — so this mesh is already presentable).
                // _fluidNbMissingMask records which of the 26 surround chunks were missing at
                // bake time. _fluidNbExpectedMask is the subset we WAIT on (will-generate AND
                // has a trigger path back: the 6 faces + 4 same-Y diagonals; up/down-ring
                // diagonals never trigger rebuilds, pre-existing). Completion triggers clear
                // their expected bit; only the one that EMPTIES the mask pays the single final
                // heal rebuild, which bakes the exact borders.
                int fnbMask = 0;
                int fnbExpected = 0;
                for (int fdy = -1; fdy <= 1; fdy++)
                {
                    for (int fdz = -1; fdz <= 1; fdz++)
                    {
                        for (int fdx = -1; fdx <= 1; fdx++)
                        {
                            ...
                            {
                                int fbitv = 1 << ((fdx + 1) * 9 + (fdy + 1) * 3 + (fdz + 1));
                                fnbMask |= fbitv;
                                if ((fdy == 0) || (fdx == 0 && fdz == 0))
                                {
                                    int fci = ChunkCenteredCoordsTo1D(chunk.chunkX_world + fdx, chunk.chunkY_world + fdy, chunk.chunkZ_world + fdz);
                                    if (fci != -1 && ShouldGenerateChunkData(fci)) fnbExpected |= fbitv;
                                }
                            }
                        }
                    }
                ...
                chunk._fluidNbExpectedMask = fnbExpected;
                ...
                if (enableDetailedTimings)
                {
                    stats_fluidSetupMs += (Time.realtimeSinceStartup - _fset0) * 1000f;
                    stats_fluidBuilds++;
                }
#endif
                return;
        ...
        // RENDER GATES: a fully-buried chunk (all voxels opaque-solid AND all 6 neighbors also
        // all-opaque-solid) has zero visible faces — skip its renderers entirely. The shared voxel
        // mesh costs ~98k vertex-shader invocations (x ~5 texture taps each) per renderer per
        // frame, so every skipped renderer matters. Conservative: a missing/unscanned/edited
        // neighbor counts as "not solid" and the chunk renders. Boundary edits trigger neighbor
        // remeshes, which re-run this gate, so exposure by digging re-enables rendering.
        bool chunkEnclosed = chunk._isAllOpaqueSolid && _AllNeighborsAllOpaqueSolid(chunk);
        if (chunk.opaqueRenderer != null)
        {
            ...
            chunk.opaqueRenderer.enabled = !chunkEnclosed;
        }
        // Cutout pass: only chunks that actually contain non-opaque content (leaves/glass/cross/
        // torch/...) need the second shared-mesh renderer. Plain terrain chunks skip it — that
        // halves their per-frame vertex load. Explicitly detach on the skip path: pooled
        // GameObjects can carry a stale cutout attach from a previous tenant.
        if (gpuVoxelCutoutMaterial != null && chunk.cutoutMeshFilter != null && chunk.cutoutRenderer != null
            && chunk._hasNonOpaqueContent && !chunkEnclosed)
        {
    ...
    // opaque-solid — i.e. this chunk's border faces are all covered. Conservative on any doubt.
    private bool _AllNeighborsAllOpaqueSolid(ChunkData chunk)
    {
        ChunkData[] neighbors = _GetCachedNeighbors(chunk);
    ...
    // 20Hz gate + cached shader property ids for the water texture animation. MC's procedural
    // water/lava textures advance once per game tick (20/sec); running the 4-blit chain per frame
    // did 4.5x the GPU passes AND scrolled flowing water 4.5x too fast at 90fps. Ticking at 20Hz
    // is both cheaper and Beta-accurate.
    private float _betaWaterAnimNextTime = 0f;
    private bool _betaWaterAnimPropsInit = false;
    private int _waterPropTime2Id, _waterPropSplashChanceId, _waterPropSplashDecayId,
                _waterPropDivisorInvId, _waterPropIsFlowModeId, _waterPropFlowScrollId;
    ...
    private void _UpdateBetaWaterAnimation()
    {
        ...
        float now = Time.time;
        if (now < _betaWaterAnimNextTime) return;
        _betaWaterAnimNextTime = now + 0.05f; // 20 ticks/sec, MC parity

        if (!_betaWaterAnimPropsInit)
        {
            _betaWaterAnimPropsInit = true;
            _waterPropTime2Id = VRCShader.PropertyToID("_Time2");
            _waterPropSplashChanceId = VRCShader.PropertyToID("_SplashChance");
            _waterPropSplashDecayId = VRCShader.PropertyToID("_SplashDecay");
            _waterPropDivisorInvId = VRCShader.PropertyToID("_DivisorInv");
            _waterPropIsFlowModeId = VRCShader.PropertyToID("_IsFlowMode");
            _waterPropFlowScrollId = VRCShader.PropertyToID("_FlowScroll");
        }

        float t = now * 73.1f;

        // Still water: evolve state then convert to color
        gpuWaterAnimMaterial.SetFloat(_waterPropTime2Id, t);
        gpuWaterAnimMaterial.SetFloat(_waterPropSplashChanceId, 0.05f);
        gpuWaterAnimMaterial.SetFloat(_waterPropSplashDecayId, 0.1f);
        gpuWaterAnimMaterial.SetFloat(_waterPropDivisorInvId, 1.0f / 3.3f);
        gpuWaterAnimMaterial.SetFloat(_waterPropIsFlowModeId, 0f);
        ...
        gpuWaterAnimMaterial.SetFloat(_waterPropFlowScrollId, 0f);
        ...
        betaWaterFlowFrame++;
        gpuWaterAnimMaterial.SetFloat(_waterPropSplashChanceId, 0.2f);
        gpuWaterAnimMaterial.SetFloat(_waterPropSplashDecayId, 0.3f);
        gpuWaterAnimMaterial.SetFloat(_waterPropDivisorInvId, 1.0f / 3.2f);
        gpuWaterAnimMaterial.SetFloat(_waterPropIsFlowModeId, 1f);
        gpuWaterAnimMaterial.SetFloat(_waterPropTime2Id, t + 17.3f);
        ...
        gpuWaterAnimMaterial.SetFloat(_waterPropFlowScrollId, (float)betaWaterFlowFrame / 16.0f);
    ...
    // EVENT-DRIVEN COLLISION GRID: the 250-cell rescan below used to run unconditionally every
    // frame (~250 full GetBlock chains — the single largest steady-state Udon cost). It now runs
    // only when the player crossed a voxel boundary, a SetBlock landed in/next to the grid
    // (_collisionGridDirty, set by _SetBlockLocal), or the periodic safety net expires (covers
    // chunk data streaming in under the player without a SetBlock — initial gen, rehydrate).
    // A falling/walking player crosses voxel boundaries continuously, so anything that could
    // make them fall through the world already forces per-frame rescans.
    ...
    private void _UpdateVoxelColliderGrid()
    {
        ...
        _collisionSafetyCountdown = _collisionSafetyCountdown - 1;
        if (!moved && !_collisionGridDirty && _collisionSafetyCountdown > 0) return;
        _collisionGridDirty = false;
        _collisionSafetyCountdown = collisionGridSafetyRescanFrames < 1 ? 1 : collisionGridSafetyRescanFrames;
        ...
        // Cached-chunk fast path: the scan is spatially coherent, so resolve the chunk only when
        // the (cx,cy,cz) key changes and read ids straight from the decompressed cache when valid
        // (the exact array GetBlock's fast path returns). Cold chunks fall back to _GetBlockLocal,
        // which preserves GetBlock semantics (decompress, GPU-resident rehydrate request).
        int colDimX = worldDimensionX, colDimY = worldDimensionY, colDimZ = worldDimensionZ;
        ...
        for (int dy = -down; dy <= up; dy++)
        {
            ...
            for (int dz = -R; dz <= R; dz++)
            {
                ...
                for (int dx = -R; dx <= R; dx++)
                {
                    ...
                    if (wy >= 0)
                    {
                        int ccx = wx >> 4, ccy = wy >> 4, ccz = wz >> 4;
                        if (ccx != lastCX || ccy != lastCY || ccz != lastCZ)
                        {
                            lastCX = ccx; lastCY = ccy; lastCZ = ccz;
                            colChunk = null; colData = null;
                            int ax = ccx + chunkOffsetX, ay = ccy + chunkOffsetY, az = ccz + chunkOffsetZ;
                            if (ax >= 0 && ax < colDimX && ay >= 0 && ay < colDimY && az >= 0 && az < colDimZ)
                            {
                                ChunkData cc = chunks_1D[(az * colDimX * colDimY) + (ay * colDimX) + ax];
                                if (cc != null && cc.isDataReady)
                                {
                                    colChunk = cc;
                                    if (cc._decompCacheValid) colData = cc._cachedDecompressedData;
                                }
                            }
                        }
                        int bid;
                        if (colData != null)
                        {
                            bid = colData[((wy & 15) << 8) | ((wz & 15) << 4) | (wx & 15)];
                        }
                        else if (colChunk != null)
                        {
                            bid = _GetBlockLocal(colChunk, wx & 15, wy & 15, wz & 15) & 0xFF;
                        }
                        else
                        {
                            bid = 0;
    ...
    private void ProcessActiveChunks()
    {
        ...
        // --- STARVATION RESCUE lane ---
        // The shared meshing loop below always starts at index 0 and breaks at its deadline, and
        // swap-remove refills freed head slots from the TAIL — so entries in the middle of the
        // list can starve indefinitely on over-budget frames (observed: player-adjacent chunks
        // stuck at prepStage=0 for 300 frames until the watchdog killed them). Guarantee progress:
        // one rotating entry gets stepped every frame under a small dedicated budget, so every
        // listed build is touched at least once every activeMeshingCount frames.
        if (activeMeshingCount > 0)
        {
            _meshRescueCursor = _meshRescueCursor + 1;
            if (_meshRescueCursor >= activeMeshingCount) _meshRescueCursor = 0;
            ChunkData rescue = activeMeshingChunks[_meshRescueCursor];
            if (rescue != null && rescue.isBuildingMesh)
            {
                float rescueDeadline = Time.realtimeSinceStartup + 0.002f; // 2ms cap (one atomic step may overshoot)
                for (int rs = 0; rs < 8; rs++)
                {
                    if (!rescue.isBuildingMesh) break;
                    if (Time.realtimeSinceStartup > rescueDeadline) break;
                    _BuildChunkMeshStep(rescue);
                }
            }
        }
#if LOGGING
        ...
        {
#if LOGGING
            float _pacL2 = enableDetailedTimings ? Time.realtimeSinceStartup : 0f;
#endif
...
#if LOGGING
        float _pacT8 = enableDetailedTimings ? Time.realtimeSinceStartup : 0f;
#endif
    ...
    private void _ScheduleDeferredInteriorMeshUpdates(float frameStart, float frameBudget)
    {
        ...
        while (scansRemaining-- > 0)
        {
            ...
            if (!chunk.isMeshDeferred && chunk._borderMissingMask != 0)
            {
                ...
                else if (_AreMaskedNeighborsReady(chunk, chunk._borderMissingMask))
                {
                    RequestChunkMeshUpdate(chunkIndex);
#if LOGGING
                    stats_rqHealInterior++;
#endif
                }
                continue;
            }
    ...
    private void _TryImmediateBorderHeal(ChunkData chunk)
    {
        ...
        // Instanced chunks read neighbors from the GPU atlas at draw time — nothing to heal.
        if (chunk._isInstancedRendered) { chunk._borderMissingMask = 0; return; }
    ...
    {
        // Box-collider mode: chunk mesh colliders are never built, so this 125-cell per-frame
        // poll can only ever discover no-op work — skip it entirely.
        if (useBoxColliderCollision) return 0;
    ...
    private void _ProcessDeferredChunkSecondaryWork(float frameStart, float frameBudget)
    {
        ...
        while (scansRemaining-- > 0)
        {
            ...
            if (!useBoxColliderCollision && chunk.pendingColliderMeshRebuild && !chunk.isBuildingMesh)
            {
                if (!_ShouldEnableChunkCollider(chunk)) continue;
                chunk.pendingColliderMeshRebuild = false;
                RequestChunkMeshUpdate(chunkIndex);
            ...
            if (!useBoxColliderCollision && nearOnlyChunkColliders && !chunk.isBuildingMesh && chunk.meshCollider != null && chunk.meshCollider.enabled && !_ShouldEnableChunkCollider(chunk))
            {
            ...
            if (!useBoxColliderCollision && chunk.pendingColliderApply && !chunk.isBuildingMesh)
            {
                if (!_ShouldEnableChunkCollider(chunk)) continue;
                chunk.pendingColliderApply = false;
                _ApplyDataToCollider(chunk);
    ...
    // DYNAMIC GENERATOR ASSIGNMENT (2026-07-14): a column's generator is chosen at column
    // START from whatever slot is actually free (PickGeneratorSlotForNewColumn), recorded in
    // _columnGenSlot, and looked up everywhere afterwards. The old fixed (cx*2+cz)%N hash
    // left generators idle whenever the next radial candidates all hashed to busy slots
    // (observed: datagen avg 1.3-1.5 of 3 generators during load) and produced the diagonal
    // frontier pattern. The map PERSISTS after completion so post-gen consumers (sibling
    // cache drain, resident repack, GetBiomeDataForChunk) still reach the generator that
    // actually made the column. Unassigned columns fall back to the old spread hash — that
    // only answers "does a cache exist" queries with the correct NO, and a routing mistake
    // is a cache miss, never terrain corruption (all generators share the seed).
    private int[] _columnGenSlot; // per column (az*worldDimensionX+ax): assigned slot, -1 = never assigned

    private int _GeneratorSlotForColumn(int cx, int cz)
    {
        int n = GetGeneratorCount();
        if (n <= 1) return 0;
        if (_columnGenSlot != null)
        {
            int ax = cx + chunkOffsetX;
            int az = cz + chunkOffsetZ;
            if (ax >= 0 && ax < worldDimensionX && az >= 0 && az < worldDimensionZ)
            {
                int mapped = _columnGenSlot[az * worldDimensionX + ax];
    ...
    private McTerrainGenerator _GeneratorForSlot(int slot)
    {
        if (slot <= 0) return terrainGenerator;
        if (extraGenerators == null || slot - 1 >= extraGenerators.Length) return null;
        return extraGenerators[slot - 1];
    }

    private McTerrainGenerator _GeneratorForColumn(int cx, int cz)
    {
        McTerrainGenerator g = _GeneratorForSlot(_GeneratorSlotForColumn(cx, cz));
        return g != null ? g : terrainGenerator;
    }

    public void AssignColumnGeneratorSlot(int cx, int cz, int slot)
    {
        if (_columnGenSlot == null) return;
        int ax = cx + chunkOffsetX;
        int az = cz + chunkOffsetZ;
        if (ax < 0 || ax >= worldDimensionX || az < 0 || az >= worldDimensionZ) return;
        _columnGenSlot[az * worldDimensionX + ax] = slot;
    }

    // Per-frame early-out memo for PickGeneratorSlotForNewColumn: "no slot free" is the
    // common answer during load, and the picker may probe dozens of candidates per cycle —
    // this makes each probe 1 field check instead of N cross-behaviour IsIdle calls.
    private int _pickSlotMemoFrame = -1;
    private bool _pickSlotAnyFree = false;

    // IN-FLIGHT COLUMN GUARD: is this column currently being generated by its mapped
    // generator? Mid-gen (pre-readback) the column has no copyable cache, so without this
    // check its sibling Y-chunks look exactly like fresh new-column candidates — the picker
    // would bind them to OTHER free slots, overwriting the map and abandoning the original
    // generator's state machine mid-column (nothing ever steps or resets an unmapped
    // mid-column machine: a permanent slot leak). The old fixed hash blocked this
    // implicitly — siblings hashed to the trigger's busy slot.
    private bool _ColumnGenerationInFlight(int cx, int cz)
    {
        if (_columnGenSlot == null) return false;
        int ax = cx + chunkOffsetX;
        int az = cz + chunkOffsetZ;
        if (ax < 0 || ax >= worldDimensionX || az < 0 || az >= worldDimensionZ) return false;
        int slot = _columnGenSlot[az * worldDimensionX + ax];
        if (slot < 0) return false;
        McTerrainGenerator g = _GeneratorForSlot(slot);
        return g != null && g.IsGeneratingColumn(cx, cz);
    }

    // Choose a generator slot for a NEW column. Preference: (1) a free slot whose column
    // cache IS this column (restart -> cache hit), (2) the first free slot whose cache is
    ...
    public int PickGeneratorSlotForNewColumn(int cx, int cz)
    {
        int n = GetGeneratorCount();
        if (n <= 0) return -1;
        if (_pickSlotMemoFrame != Time.frameCount)
        {
            _pickSlotMemoFrame = Time.frameCount;
            _pickSlotAnyFree = false;
            for (int s = 0; s < n; s++)
            {
                if (_GenSlotIsFreeForNewColumn(s)) { _pickSlotAnyFree = true; break; }
            }
        }
        if (!_pickSlotAnyFree) return -1;
        // IN-FLIGHT GUARD after the no-free-slot early-out (both answer -1; this one costs a
        // cross-behaviour call for mapped candidates, and "no slot free" is the common case
        // during load). It only needs to run before a pass can return a slot.
        if (_ColumnGenerationInFlight(cx, cz)) return -1;
        // Pass 1: cache affinity — a free generator already holding THIS column's cache.
        for (int s = 0; s < n; s++)
        {
    ...
    public bool StartChunkDataGeneration(int chunkIndex)
    {
        ...
        // GPU-RESIDENT (#2 step 2): compute the per-column skip-readback policy into a LOCAL.
        // DYNAMIC ASSIGNMENT: the field is written ONLY onto the generator we actually START
        // below — with dynamic slots, `gen` here can be a hash-fallback generator that is
        // BUSY mid-generation of a FOREIGN column, and its state machine reads the field
        // LIVE at its finalize fork; the old unconditional write here would flip that
        // foreign column's resident/readback policy mid-flight.
        bool skipReadbackPolicy = _ColumnShouldSkipReadback(chunk);
        if (skipReadbackPolicy && gen.CanRepackGpuResidentColumn(chunk.chunkX_world, chunk.chunkZ_world))
        {
        ...
        {
            // DYNAMIC GENERATOR ASSIGNMENT: fresh column — nothing holds data for it, so bind
            // it to whichever slot is free RIGHT NOW instead of the fixed hash. The picker
            // gate ran in the same worker iteration, so a slot is normally available; -1 is
            // a same-cycle race safety net — park the chunk unreserved (the exact state
            // StepChunkDataGeneration's busy-generator path already handles: it restarts the
            // column when its mapped generator frees).
            int freshSlot = PickGeneratorSlotForNewColumn(chunk.chunkX_world, chunk.chunkZ_world);
            if (freshSlot >= 0)
            {
                AssignColumnGeneratorSlot(chunk.chunkX_world, chunk.chunkZ_world, freshSlot);
                McTerrainGenerator freshGen = _GeneratorForSlot(freshSlot);
                if (freshGen != null) gen = freshGen;
                gen.gpuSkipReadbackForColumn = skipReadbackPolicy;
    ...
    public void StepChunkDataGeneration(ChunkData chunk)
    {
        ...
        else
        {
            ...
            if (useGpuWorldgen && !gen.IsGeneratingChunk(chunk.chunkX_world, chunk.chunkY_world, chunk.chunkZ_world))
            {
                if (gen.IsIdle() && !GenBlocksNewColumnForCacheDrain(gen, _GeneratorSlotForColumn(chunk.chunkX_world, chunk.chunkZ_world), chunk.chunkX_world, chunk.chunkZ_world))
                {
                    ...
                    // DYNAMIC ASSIGNMENT: record the slot in the map (idempotent when already
                    // mapped; records the hash-fallback slot for a parked unmapped column) so
                    // _ColumnGenerationInFlight sees this restart and siblings can't rebind
                    // the column to another generator mid-flight.
                    AssignColumnGeneratorSlot(chunk.chunkX_world, chunk.chunkZ_world, _GeneratorSlotForColumn(chunk.chunkX_world, chunk.chunkZ_world));
                    gen.gpuSkipReadbackForColumn = _ColumnShouldSkipReadback(chunk);
                    gen.StartChunkGeneration(chunk.chunkX_world, chunk.chunkY_world, chunk.chunkZ_world);
                }
                else
                {
                    // DYNAMIC ASSIGNMENT rescue: the pinned generator holds nothing usable for
                    // this column (TryCopy already failed above) and is busy/drain-blocked —
                    // any FREE generator can legally take the column (shared seed). Re-pick
                    // instead of waiting for this specific slot; the in-flight guard inside
                    // the pick keeps genuinely mid-gen columns waiting for their cache, so
                    // this only rebinds columns whose generation was lost (cache stomped).
                    int rescueSlot = PickGeneratorSlotForNewColumn(chunk.chunkX_world, chunk.chunkZ_world);
                    if (rescueSlot >= 0)
                    {
                        McTerrainGenerator rescueGen = _GeneratorForSlot(rescueSlot);
                        if (rescueGen != null)
                        {
                            AssignColumnGeneratorSlot(chunk.chunkX_world, chunk.chunkZ_world, rescueSlot);
                            rescueGen.gpuSkipReadbackForColumn = _ColumnShouldSkipReadback(chunk);
                            rescueGen.StartChunkGeneration(chunk.chunkX_world, chunk.chunkY_world, chunk.chunkZ_world);
                        }
                    }
                }
        ...
        if (isComplete)
        {
                ...
                && (generatedData == null || !gen.IsLastWorkedChunk(chunk.chunkX_world, chunk.chunkY_world, chunk.chunkZ_world)))
            {
                ...
                if (gen.IsIdle() && !GenBlocksNewColumnForCacheDrain(gen, _GeneratorSlotForColumn(chunk.chunkX_world, chunk.chunkZ_world), chunk.chunkX_world, chunk.chunkZ_world))
                {
                    // DYNAMIC ASSIGNMENT: map write for the same reason as the wait-path
                    // restart above — the restart must be visible to _ColumnGenerationInFlight.
                    AssignColumnGeneratorSlot(chunk.chunkX_world, chunk.chunkZ_world, _GeneratorSlotForColumn(chunk.chunkX_world, chunk.chunkZ_world));
                    gen.gpuSkipReadbackForColumn = _ColumnShouldSkipReadback(chunk);
                    gen.StartChunkGeneration(chunk.chunkX_world, chunk.chunkY_world, chunk.chunkZ_world);
                }
            ...
            // EVENT-DRIVEN RESIDENT MAINTAIN: announce this freshly generated chunk to the atlas
            // sync queue so _GpuMaintainResidentChunks doesn't have to ring-scan to discover it.
            _GpuQueueResidentSync(chunk);
#if LOGGING
    ...
    // Incremented on every real block change (see _SetBlockLocal). Consumers compare snapshots to
    // skip re-sampling the world when nothing can have changed.
    [System.NonSerialized] public int worldEditCounter;
    ...
    public void TriggerNeighborMeshRebuilds(ChunkData chunk, bool interactionPriority)
    {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            // just the renderer toggles inline.
            // WATERY neighbors — FLUID ONE-HEAL: their water built immediately with missing
            // chunks culled; a completion whose missing-bit is set just clears its EXPECTED bit,
            // and only the completion that empties the expected mask (or arrives after it is
            // already empty — range expansion) pays the single final heal rebuild.
            bool fluidNeedsRebuild = false;
            if (neighborChunk._hasWaterBlocks)
            {
                int fbit = 1 << ((1 - neighbor_dx_offsets[i]) * 9 + (1 - neighbor_dy_offsets[i]) * 3 + (1 - neighbor_dz_offsets[i]));
                if ((neighborChunk._fluidNbMissingMask & fbit) != 0)
                {
                    neighborChunk._fluidNbExpectedMask = neighborChunk._fluidNbExpectedMask & ~fbit;
                    fluidNeedsRebuild = neighborChunk._fluidNbExpectedMask == 0;
                }
            }
        ...
        // FLUID ONE-HEAL: same rule as the side loop — clear our expected bit in the diagonal
        // watery chunk; only the completion that EMPTIES its expected mask requests the single
        // final heal (if it is mid-build right then, flag the existing followup rebuild so the
        // final heal is not lost). Bit = completing chunk relative to the diagonal:
        // (1-dx)*9 + 3 + (1-dz).
        ...
        {
            bool dReb = interactionPriority;
            if (!dReb && (diagonalChunk._fluidNbMissingMask & (1 << 23)) != 0)
            {
                diagonalChunk._fluidNbExpectedMask = diagonalChunk._fluidNbExpectedMask & ~(1 << 23);
                if (diagonalChunk._fluidNbExpectedMask == 0)
                {
                    if (!diagonalChunk.isMeshDeferred && !diagonalChunk.isBuildingMesh) dReb = true;
                    else if (diagonalChunk.isBuildingMesh) diagonalChunk.pendingChunkMeshRebuild = true;
                }
            }
            if (dReb)
            {
        ...
        {
            bool dReb = interactionPriority;
            if (!dReb && (diagonalChunk._fluidNbMissingMask & (1 << 21)) != 0)
            {
                diagonalChunk._fluidNbExpectedMask = diagonalChunk._fluidNbExpectedMask & ~(1 << 21);
                if (diagonalChunk._fluidNbExpectedMask == 0)
                {
                    if (!diagonalChunk.isMeshDeferred && !diagonalChunk.isBuildingMesh) dReb = true;
                    else if (diagonalChunk.isBuildingMesh) diagonalChunk.pendingChunkMeshRebuild = true;
        ...
        {
            bool dReb = interactionPriority;
            if (!dReb && (diagonalChunk._fluidNbMissingMask & (1 << 5)) != 0)
            {
                diagonalChunk._fluidNbExpectedMask = diagonalChunk._fluidNbExpectedMask & ~(1 << 5);
                if (diagonalChunk._fluidNbExpectedMask == 0)
                {
                    if (!diagonalChunk.isMeshDeferred && !diagonalChunk.isBuildingMesh) dReb = true;
                    else if (diagonalChunk.isBuildingMesh) diagonalChunk.pendingChunkMeshRebuild = true;
        ...
        {
            bool dReb = interactionPriority;
            if (!dReb && (diagonalChunk._fluidNbMissingMask & (1 << 3)) != 0)
            {
                diagonalChunk._fluidNbExpectedMask = diagonalChunk._fluidNbExpectedMask & ~(1 << 3);
                if (diagonalChunk._fluidNbExpectedMask == 0)
                {
                    if (!diagonalChunk.isMeshDeferred && !diagonalChunk.isBuildingMesh) dReb = true;
                    else if (diagonalChunk.isBuildingMesh) diagonalChunk.pendingChunkMeshRebuild = true;
    ...
    public void ForceCompleteStuckMesh(int chunkIndex)
    {
        ...
        // DIAGNOSIS + ORPHAN RESCUE: determine whether this "stuck" build is actually in
        // activeMeshingChunks. A valid in-progress build that is NOT listed can never be stepped
        // (that is a lost/orphaned entry, not a wedge) — re-list it and let it finish normally
        // instead of killing it. A listed one that still hit the watchdog was starved or wedged;
        // fall through to the release below (the rotating rescue lane should make this rare).
        int listIndex = -1;
        for (int mi = 0; mi < activeMeshingCount; mi++)
        {
            if (activeMeshingChunks[mi] == chunk) { listIndex = mi; break; }
        }
        if (listIndex < 0 && chunk.isBuildingMesh && (chunk._gpuPrepActive || chunk._gpuFaceBuildActive)
            && activeMeshingCount < MAX_ACTIVE_CHUNKS)
        {
            int slot = activeMeshingCount;
            activeMeshingChunks[slot] = chunk;
            activeMeshingCount = slot + 1;
            Debug.LogWarning("[McWorld] Mesh watchdog RE-LISTED an orphaned in-progress build at chunk ("
                + chunk.chunkX_world + "," + chunk.chunkY_world + "," + chunk.chunkZ_world
                + ") prepActive=" + chunk._gpuPrepActive + " prepStage=" + chunk._gpuPrepStage
                + " faceBuildActive=" + chunk._gpuFaceBuildActive
                + " — it was building but missing from activeMeshingChunks (count=" + slot + ")");
            return;
        }
        ...
        Debug.LogWarning("[McWorld] Mesh watchdog force-released a stuck build at chunk ("
            ...
            + " prepActive=" + chunk._gpuPrepActive + " prepStage=" + chunk._gpuPrepStage
            + " meshPending=" + chunk._gpuMeshPending + " dataReady=" + chunk.isDataReady
            + " allAir=" + chunk._isAllAir + " borderMask=" + chunk._borderMissingMask
            + " hasWater=" + chunk._hasWaterBlocks
            + " listIndex=" + listIndex + "/" + activeMeshingCount
            + " prepActive2=" + chunk._gpuPrepActive + " prepStage2=" + chunk._gpuPrepStage);
        if (chunk._gpuFaceBuildActive) _ReleaseGpuFaceBuildBuffer(chunk);
        _ReleaseMeshPool(chunk);
        chunk._gpuFaceBuildActive = false;
        chunk._gpuMeshPending = false;
        chunk._gpuPrepActive = false;   // hygiene: a half-done prep must not leak into the next build
        chunk._gpuPrepData = null;
        chunk.isBuildingMesh = false;
        chunk.interactionMeshPriority = false;
    }
    ...
    public void BuildChunkMesh(int chunkIndex)
    {
            ...
            && (chunk.pendingChunkMeshRebuild || chunk.interactionMeshPriority || _IsChunkWithinMeshRenderDistance(chunk)))
        {
            ...
            if (prepData != null && !chunk._isAllAir && !needsExactCpuGeometry)
            {
                ...
                // ORPHAN FIX: never start a phantom build. If the active list is full (observed
                // intra-frame during rebuild storms: sampled max 63), a dropped add left the chunk
                // isBuildingMesh=true but unsteppable until the watchdog. Abort cleanly instead —
                // the deferred sweep re-queues it once the list drains.
                if (activeMeshingCount >= MAX_ACTIVE_CHUNKS)
                {
                    chunk._gpuPrepActive = false;
                    chunk._gpuPrepData = null;
        ...
        chunk._gpuMeshPending = false;
        chunk.pendingColliderApply = false;
        // Box-collider mode never emits chunk collision geometry (see the useBoxColliderCollision
        // gates in the face emitters) — flagging a collider rebuild would only schedule a full
        // remesh that produces nothing. Only the mesh-collider mode needs the deferred rebuild.
        chunk.pendingColliderMeshRebuild = !useBoxColliderCollision && !_ShouldEnableChunkCollider(chunk);
        if (interactionPriority)
        {
        ...
        {
            _ApplyEmptyMesh(chunk);
            // REBUILD-STORM FIX: an air chunk's mesh is empty REGARDLESS of neighbor state, so a
            // border-missing mask on it is meaningless — but this early-out used to complete
            // WITHOUT clearing it, so the border-heal sweeps re-requested the same air chunks
            // forever (hundreds of [empty] rebuilds per window) once their neighbors generated.
            chunk._borderMissingMask = 0;
            chunk._renderMeshApplied = true;
#if LOGGING
            _RecordMeshBuildCompletion(chunk, false, true);
#endif
            chunk.isBuildingMesh = false;
        ...
        {
            // ORPHAN FIX: full list — abort instead of leaving an unsteppable phantom build.
            chunk.isBuildingMesh = false;
            chunk.interactionMeshPriority = false;
            MarkChunkMeshDeferred(chunkIndex);
        }
    ...
    {
        // Decompress self and any ready neighbors once. Neighbors use the RAW variant: only their
        // BYTES are sampled here (border culling / corner heights) — their derived caches are
        // materialized by their own render-prep, not as a side effect of being someone's neighbor.
        chunk._decompSelf = _GetDecompressedData(chunk);
        chunk._decompNX = (chunk.neighborNX != null && chunk.neighborNX.isDataReady) ? _GetDecompressedDataRaw(chunk.neighborNX) : null;
        chunk._decompPX = (chunk.neighborPX != null && chunk.neighborPX.isDataReady) ? _GetDecompressedDataRaw(chunk.neighborPX) : null;
        chunk._decompNY = (chunk.neighborNY != null && chunk.neighborNY.isDataReady) ? _GetDecompressedDataRaw(chunk.neighborNY) : null;
        chunk._decompPY = (chunk.neighborPY != null && chunk.neighborPY.isDataReady) ? _GetDecompressedDataRaw(chunk.neighborPY) : null;
        chunk._decompNZ = (chunk.neighborNZ != null && chunk.neighborNZ.isDataReady) ? _GetDecompressedDataRaw(chunk.neighborNZ) : null;
        chunk._decompPZ = (chunk.neighborPZ != null && chunk.neighborPZ.isDataReady) ? _GetDecompressedDataRaw(chunk.neighborPZ) : null;
    }
    ...
    private byte _GetFluidRenderBlock(ChunkData originChunk, int localX, int localY, int localZ)
    {
        ...
        // MISSING = STILL WATER (fluid rendering only): a missing/not-ready chunk used to read
        // as air, which baked "water wall" faces at the frontier and made every initial build
        // throwaway. Reading it as still water CULLS those faces instead (and keeps corner
        // heights at the flat 8/9), so the immediate build is already presentable; the one
        // final heal (expected-mask empty) bakes the exact borders. At the permanent data
        // horizon this renders no side walls — faces there never existed in the final state
        // anyway once neighbors load.
        if (sampleChunk == null || !sampleChunk.isDataReady) return BLOCK_WATER_STILL;
    ...
    private void _AddWaterBlocksSliced(ChunkData chunk, int zStart, int zCount)
    {
        ...
        // Trust the fluid span only when it was computed for the CURRENT data version (the atomic
        // derive path after edits doesn't fill it — fall back to the occupied-bounds scan there).
        byte[] colFluid = (chunk._columnFluidMask != null && chunk._columnFluidMaxY != null
            && chunk._columnFluidMaskVersion == chunk._cachedDataVersion) ? chunk._columnFluidMask : null;
        byte[] colFluidMax = colFluid != null ? chunk._columnFluidMaxY : null;
        byte[] ringTbl = _fluidSurfRingTable;
        ...
        for (int z = zStart; z < zEnd; z++)
        {
            ...
            for (int x = 0; x < chunkSizeXZ; x++)
            {
                ...
                if (colFluid != null)
                {
                    int fMin = colFluid[columnIndex];
                    if (fMin == 255) continue; // no fluid anywhere in this column
                    yFrom = fMin;
                    yTo = colFluidMax[columnIndex];
                }
                else
                {
                ...
                for (int y = yFrom; y <= yTo; y++)
                {
                    ...
                    {
                        // Missing (< 0) reads as still water for WATER cells — mirrors the full
                        // path's missing=BLOCK_WATER_STILL sentinel exactly (faces culled).
                    ...
                    // Opaque-cube culls for the non-top faces (mirrors _IsOpaqueCubeForWater):
                    // sides/bottom cull against opaque cubes too — sand/stone floors and walls,
                    // which is MOST real water (coasts, shallows, ponds). The first V2 build's
                    // counters proved same-fluid-only preconditions missed ~85% of fluid cells.
                    bool cDn = sDn || (nDn > 0 && (clsTable[nDn] & DBC_OPAQUE) != 0);
                    bool cXP = sXP || (nXP > 0 && (clsTable[nXP] & DBC_OPAQUE) != 0);
                    bool cXN = sXN || (nXN > 0 && (clsTable[nXN] & DBC_OPAQUE) != 0);
                    bool cZP = sZP || (nZP > 0 && (clsTable[nZP] & DBC_OPAQUE) != 0);
                    bool cZN = sZN || (nZN > 0 && (clsTable[nZN] & DBC_OPAQUE) != 0);

                    // FAST PATH 1: every face culled -> zero quads. The top face only ever culls
                    // against same fluid (or ice over water) — never against opaque.
                    if ((sUp || (!cellIsLava && nUp == BLOCK_ICE)) && cDn && cXP && cXN && cZP && cZN)
                    {
#if LOGGING
                        stats_fluidCellsEnclosed++;
#endif
                    ...
                    if (!cellIsLava && cDn && cXP && cXN && cZP && cZN)
                    {
                        // FAST PATH 2: still-water surface (ocean, shallows, ponds) — only the
                        // top face renders, all 4 corner heights are the constant 8/9 and there
                        // is no flow. Exact preconditions mirroring the full path's math: every
                        // ring cell (3x3 at this Y) is WATER (meta 0, checked below) or an
                        // opaque SOLID cube (the corner sampler skips solid samples entirely;
                        // opaque-and-solid is the conservative exact subset — see the ring
                        // table), and no cell of the 3x3 above is water (would force a 1.0
                        ...
                        if (surfHead && colInterior)
                        {
                            bool ringOk =
                                ...
                                (nZP < 0 || ringTbl[nZP] != 0) &&
                                (nZN < 0 || ringTbl[nZN] != 0) &&
                                ringTbl[data[i + 17]] != 0 &&  // x+1, z+1
                                ringTbl[data[i + 15]] != 0 &&  // x-1, z+1
                                ringTbl[data[i - 15]] != 0 &&  // x+1, z-1
                                ringTbl[data[i - 17]] != 0;    // x-1, z-1
                            if (ringOk)
                            {
    ...
    private byte[] _GetDecompressedData(ChunkData chunk)
    {
        ...
        chunk._decompCacheValid = true;
        // Re-derive only when the derived caches are actually stale for this data version — a
        // decomp-cache miss on a derived-fresh chunk (e.g. fluid setup touching a neighbor whose
        // prep already ran) paid a pointless 4-15ms atomic 4096-voxel scan here.
        if (chunk._derivedForDataVersion != chunk._cachedDataVersion)
            _RefreshChunkDerivedData(chunk, decompressed);
        return decompressed;
    }

    // Bytes-only decompress for callers that sample NEIGHBOR block data (fluid border culling):
    // identical to _GetDecompressedData minus the atomic derived-data refresh. During load every
    // freshly finalized HOMOGENEOUS chunk (all-air above the ocean, all-water deeps, all-stone)
    // is born with an invalid decomp cache AND deferred derived data, so the first watery build
    // that touched it as a neighbor paid decompress + a 4-15ms atomic derive scan it never needed
    // — the neighbor's own render-prep derives it (time-sliced) when its turn comes.
    private byte[] _GetDecompressedDataRaw(ChunkData chunk)
    {
        if (chunk == null) return null;
        if (chunk._decompCacheValid && chunk._cachedDecompressedData != null) return chunk._cachedDecompressedData;
        if (chunk._isGpuResident)
        {
            if (!chunk._gpuMirrorRehydratePending) _GpuRequestRehydrate(chunk);
            return chunk._cachedDecompressedData;
    ...
    private void _SetBlockLocal(ChunkData chunk, int x, int y, int z, byte blockType, bool updateMesh, bool interactionPriority)
    {
        ...
        // Monotonic edit counter — cheap "did ANY block change since frame X" signal for
        // per-frame consumers that cache world samples (McMovement env sensing, etc.).
        worldEditCounter = worldEditCounter + 1;

        // COLLISION DIRTY: an edit inside (or adjacent to) the player's box-collider grid wakes
        // the event-driven grid rescan (see _UpdateVoxelColliderGrid).
        if (useBoxColliderCollision)
        {
            int cgdx = ((chunk.chunkX_world << 4) + x) - _collisionPlayerVoxelX;
            int cgdy = ((chunk.chunkY_world << 4) + y) - _collisionPlayerVoxelY;
            int cgdz = ((chunk.chunkZ_world << 4) + z) - _collisionPlayerVoxelZ;
            int cgr = collisionGridRadiusXZ + 1;
            if (cgdx <= cgr && cgdx >= -cgr && cgdz <= cgr && cgdz >= -cgr
                && cgdy <= collisionGridUp + 1 && cgdy >= -(collisionGridDown + 1))
            {
    ...
    private void _EnsureDerivedBlockClassTable()
    {
        ...
        // FLUID SURFACE RING TABLE: 1 = this block id, appearing in the 3x3 ring around a
        // still-water surface cell, keeps the corner-height average at the constant 8/9 —
        // i.e. it is WATER (contributes the meta-0 term; metas checked separately) or an
        // opaque SOLID cube (the corner sampler skips solid samples entirely). Air or any
        // non-solid/non-opaque block would pull a corner height away from the constant, so
        // those cells force the exact full path.
        byte[] r = new byte[256];
        for (int id = 1; id < 256; id++)
        {
            byte b = (byte)id;
            if (_IsWaterBlock(b)) { r[id] = 1; continue; }
            bool opaque = visibilityCache != null && id < visibilityCache.Length && visibilityCache[id] == BlockVisibilityType.Opaque;
            if (opaque && _IsBlockSolid(b)) r[id] = 1;
        }
        _fluidSurfRingTable = r;
    }
    ...
    private void _RefreshChunkDerivedData(ChunkData chunk, byte[] fullData)
    {
        ...
        if (chunk._chunkDataKind == ChunkData.CHUNK_KIND_HOMOGENEOUS)
        {
            ...
            // Homogeneous lava: only the top layer can ever emit embers (needs air above, checked
            // again at emission time against the chunk above). Register those 256 cells.
            if (_IsLavaBlock(hb))
            {
                ...
                int topY = chunkSizeY - 1;
                int emitIdx = 0;
                for (int hz = 0; hz < chunkSizeXZ; hz++)
                {
                    for (int hx = 0; hx < chunkSizeXZ; hx++)
                    {
                        chunk._ambientEmitterPacked[emitIdx] = (hb << 24) | (hx << 16) | (topY << 8) | hz;
                        emitIdx++;
                    }
                }
                chunk._ambientEmitterCount = emitIdx;
            }
            if (hb == 0)
            {
                chunk._isAllAir = true;
            ...
            else
            {
                ...
                bool hbOpaque = visibilityCache != null && hb < visibilityCache.Length
                    && visibilityCache[hb] == BlockVisibilityType.Opaque;
                chunk._hasNonOpaqueContent = !hbOpaque;
                chunk._isAllOpaqueSolid = hbOpaque;
    ...
    private bool _RefreshChunkDerivedDataSliced(ChunkData chunk, byte[] fullData, int yStart, int yCount)
    {
        ...
        if (yStart == 0)
        {
            ...
            if (chunk._columnFluidMaxY == null || chunk._columnFluidMaxY.Length != columnCount)
                chunk._columnFluidMaxY = new byte[columnCount];
            // Homogeneous chunks are cheap — do them in one shot (no slicing needed).
            if (chunk._chunkDataKind == ChunkData.CHUNK_KIND_HOMOGENEOUS)
            {
                _RefreshChunkDerivedData(chunk, fullData);
                // Uniform content -> uniform fluid span, straight from the single block id.
                bool homogFluid = (_derivedBlockClass[fullData[0]] & DBC_FLUID) != 0;
                byte homogMin = homogFluid ? (byte)0 : (byte)255;
                byte homogMax = homogFluid ? (byte)15 : (byte)255;
                for (int i = 0; i < columnCount; i++)
                {
                    chunk._columnFluidMask[i] = homogMin;
                    chunk._columnFluidMaxY[i] = homogMax;
                }
            ...
            for (int i = 0; i < columnCount; i++)
            {
                ...
                chunk._columnFluidMask[i] = 255;
                chunk._columnFluidMaxY[i] = 255;
            }
        }
        ...
        byte[] colFluidMaxArrD = chunk._columnFluidMaxY;
        ...
        for (int y = yStart; y < yEnd; y++)
        {
            ...
            for (int z = 0; z < chunkSizeXZ; z++)
            {
                ...
                for (int x = 0; x < chunkSizeXZ; x++)
                {
                    ...
                    {
                        locHasWater = true;
                        // Fluid Y-span per column (y ascends across slices: first hit = min).
                        if (colFluidArrD[columnIndex] == 255) colFluidArrD[columnIndex] = (byte)y;
                        colFluidMaxArrD[columnIndex] = (byte)y;
                    }
    ...
    private void LogAggregateStats()
    {
        ...
        if (stats_frameCount > 0)
        {
            ...
            if (enableDetailedTimings)
            {
                ...
                logBuilder.AppendLine($"  Fluid detail (V2): builds {stats_fluidBuilds}, setup {stats_fluidSetupMs:F1}ms, cells enclosed/surface/full {stats_fluidCellsEnclosed}/{stats_fluidCellsSurf}/{stats_fluidCellsFull}");
            }
            if (enableAdaptiveBudgets)
            {
        ...
        stats_pacReadbackMs = 0f;
        stats_pacDataGenMs = 0f;
        stats_pacNearMeshMs = 0f;
        ...
        stats_fluidSetupMs = 0f;
        stats_fluidBuilds = 0;
        stats_fluidCellsEnclosed = 0;
        stats_fluidCellsSurf = 0;
        stats_fluidCellsFull = 0;
